...
interface DefectWorkspaceProps {
  ...
  reject: number;
  /** Production Dipping: one defect grid per trolley. */
  trolleyMode?: boolean;
  trolleyCount?: number;
  trolleyDefects?: TrolleyDefectRow[];
  onSetTrolleyDefectQty?: (trolleyIndex: number, key: string, n: number | null) => void;
}
...
export default function DefectWorkspace({
  ...
  const perTrolley = trolleyMode && trolleyCount > 0;
  ...
  return (
    ...
    >
      ...
      >
        <div>
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            ...
            >
              {perTrolley
                ? `Defect Breakdown — ${trolleyCount} trolleys`
            ...
            >
              {perTrolley
                ? `${trolleyCount} × ${activeDefects.length} codes`
                : `${activeDefectCount} active / ${activeDefects.length} codes`}
            </span>
          </div>
          <span style={{ fontSize: 12, color: "var(--text-3)" }}>
            {perTrolley
              ? `Each trolley has the same defect codes. All trolley totals together must explain Rejected (${reject}).`
      ...
      {trolleyMode && trolleyCount <= 0 ? (
        ...
        >
          Enter the number of trolleys above. Defect codes then repeat once per trolley
          (Trolley 1, Trolley 2, Trolley 3, …).
        </div>
      ) : perTrolley ? (
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          {Array.from({ length: trolleyCount }, (_, i) => {
            const row = trolleyDefects[i] ?? {};
            const rowSum = rowDefectSum(row);
            return (
              <div
                key={`trolley-${i}`}
                style={{
              ...
              >
                ...
                >
                  ...
                  <div
                    style={{
                      fontSize: 11.5,
                      fontFamily: "var(--font-mono)",
                      color: rowSum > 0 ? "var(--text-2)" : "var(--text-3)",
                      fontWeight: 600,
                    }}
                  >
                    {rowSum} pcs
                ...
                <DefectTileGrid
                  ...
                  values={row}
                  filter={filter}
                  onSet={(key, n) => onSetTrolleyDefectQty?.(i, key, n)}
                  trolleyLabel={`Trolley ${i + 1}`}
                />
              </div>
            );
          })}
...
function DefectTileGrid({
  ...
  visibleDefects: { d: DefectDef; index: number }[];
  values: Record<string, number>;
  filter: string;
  onSet: (key: string, n: number | null) => void;
  trolleyLabel?: string;
}) {
  ...
  return (
    ...
    >
      {visibleDefects.map(({ d, index }) => {
        ...
        const aria = trolleyLabel
          ? `${trolleyLabel} defect count for ${d.key} (${title})`
          : `Defect count for ${d.key} (${title})`;
