...
export default function ShiftQueueTable({
  ...
  const [ledgerFilter, setLedgerFilter] = useState<"all" | "synced" | "pending">("all");
  ...
  }, [saved, ledgerFilter, categoryFilter, typeFilter, sortOrder]);

  const renderBatchRow = (rec: ShiftBatchRecord) => {
  ...
  const onLedgerRows = filteredSaved.filter((r) => r.synced);
  const notOnLedgerRows = filteredSaved.filter((r) => !r.synced);
  ...
  return (
    ...
    >
            ...
            <div style={{ display: "flex", gap: 6, alignItems: "center", flexWrap: "wrap" }}>
              ...
              <button
                ...
                style={{
                  ...
                  padding: "3px 9px",
                  borderRadius: 999,
                  background: ledgerFilter === "all" ? "var(--surface-3)" : "var(--surface-2)",
                ...
                title="Show all batches in current shift log"
              >
                All ({saved.length})
              </button>
              ...
              <button
                ...
                style={{
                  ...
                  padding: "3px 9px",
                  borderRadius: 999,
                  background: ledgerFilter === "synced"
                    ? "var(--positive)"
                    : "var(--positive-weak)",
                  color: ledgerFilter === "synced"
                    ? "#ffffff"
                    : "var(--positive)",
                  border: ledgerFilter === "synced"
                    ? "1px solid var(--positive)"
                    : "1px solid color-mix(in srgb, var(--positive) 30%, transparent)",
                  boxShadow: ledgerFilter === "synced" ? "0 0 0 2px color-mix(in srgb, var(--positive) 25%, transparent)" : "none",
                  cursor: "pointer",
                  display: "flex",
                  alignItems: "center",
                  gap: 5,
                ...
                title="Click to filter batches confirmed on ledger"
              >
                <span style={{ fontSize: 9 }}>●</span>
                <span>{syncedCount} on ledger</span>
              </button>
              ...
              <button
                ...
                style={{
                  ...
                  padding: "3px 9px",
                  borderRadius: 999,
                  background: ledgerFilter === "pending"
                    ? "var(--status-warn, #d97706)"
                    : "var(--warning-weak)",
                  color: ledgerFilter === "pending"
                    ? "#ffffff"
                    : "var(--status-warn, #d97706)",
                  ...
                  boxShadow: ledgerFilter === "pending" ? "0 0 0 2px color-mix(in srgb, var(--status-warn, #d97706) 25%, transparent)" : "none",
                  cursor: "pointer",
                  display: "flex",
                  alignItems: "center",
                  gap: 5,
                ...
                title="Click to filter batches not yet on ledger (pending sync)"
              >
                <span style={{ fontSize: 9 }}>●</span>
                <span>{pendingCount} not on ledger</span>
              </button>

              {/* Grouping Toggle */}
              {saved.length > 0 && ledgerFilter === "all" && (
                <button
                  type="button"
                  ...
                  title="Group table into On Ledger and Not on Ledger sections"
                >
      ...
      {saved.length === 0 ? (
        ...
        >
          {ledgerFilter === "synced"
            ? "No batches currently on ledger for this shift."
            : ledgerFilter === "pending"
              ? "All batches are synced to the ledger (0 pending)."
        ...
        <div style={{ overflowX: "auto", borderRadius: "var(--radius-md, 8px)", border: "1px solid var(--border)" }}>
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
            ...
            <tbody>
              {groupByLedger && ledgerFilter === "all" ? (
                <>
                  {onLedgerRows.length > 0 && (
                    <>
                      <tr style={{ background: "var(--surface-3)", borderTop: "1px solid var(--border)", borderBottom: "1px solid var(--border)" }}>
                        <td colSpan={12} style={{ padding: "8px 14px", fontWeight: 700, fontSize: 11.5, color: "var(--positive)" }}>
                          <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                            <span style={{ fontSize: 9 }}>●</span>
                            <span>On Ledger — Confirmed ({onLedgerRows.length} {onLedgerRows.length === 1 ? "batch" : "batches"})</span>
                          </div>
                        </td>
                      </tr>
                      {onLedgerRows.map(renderBatchRow)}
                    </>
                  )}

                  {notOnLedgerRows.length > 0 && (
                    <>
                      <tr style={{ background: "var(--surface-3)", borderTop: "1px solid var(--border)", borderBottom: "1px solid var(--border)" }}>
                        <td colSpan={12} style={{ padding: "8px 14px", fontWeight: 700, fontSize: 11.5, color: "var(--status-warn, #d97706)" }}>
                          <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                            <span style={{ fontSize: 9 }}>●</span>
                            <span>Not on Ledger — Pending Sync ({notOnLedgerRows.length} {notOnLedgerRows.length === 1 ? "batch" : "batches"})</span>
                          </div>
                        </td>
                      </tr>
                      {notOnLedgerRows.map(renderBatchRow)}
                    </>
                  )}
                </>
              ) : (
                filteredSaved.map(renderBatchRow)
              )}
            </tbody>
          </table>
        </div>
